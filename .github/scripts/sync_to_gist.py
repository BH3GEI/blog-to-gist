        ...
        for idx, post in enumerate(posts, 1):
            try:
                ...
                if not all([title, date, file_path]):
                    logger.warning(f"#{idx} 数据不完整: {post}")
                    continue
                ...
                filename = f"{date}-{os.path.basename(file_path)}"

                # 使用 InputFileContent 包装内容
                file_content = InputFileContent(full_content)
                ...
                logger.error(f"#{idx} 处理失败: {title} - {str(e)}", exc_info=True)
                continue
...
#         test_content = "这是一个测试用的 Gist 内容，用于验证 Gist 创建是否成功。"
#         full_content = format_gist_content(test_title, test_date, test_content)
#         filename = "test.md"
...
#         logger.error(f"测试 Gist 创建失败 - 状态码: {ge.status}, 数据: {ge.data}")
#     except Exception as e:
#         logger.critical(f"测试 Gist 创建失败: {str(e)}", exc_info=True)
#         raise
